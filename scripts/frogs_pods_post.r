...
POSTVEC<-matrix(Prior,ncol=89,byrow=T)
MZ_OBS<- scan(("pods.obs"),skip=0,nlines=1)
OBS<-matrix(MZ_OBS,ncol=89,byrow=T)
...
Z1noLL6 <- try(abc(OBS[1,c(14:25,30:33)], POSTVEC[,2], POSTVEC[,c(14:25,30:33)], tol=0.1,method="neuralnet"),T)
...
Z1noLL8 <- try(abc(OBS[1,c(14:25,30:33)], POSTVEC[,5], POSTVEC[,c(14:25,30:33)], tol=0.1,method="loclinear"),T)
...
write(OBS, file=paste("all_pods.obs"),ncol=89,append=T)
