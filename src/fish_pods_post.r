...
POSTVEC<-matrix(Prior,ncol=68,byrow=T)
MZ_OBS<- scan(("pods.obs"),skip=0,nlines=1)
OBS<-matrix(MZ_OBS,ncol=68,byrow=T)
...
Z1noLL6 <- try(abc(OBS[1,c(12:20,24:26)], POSTVEC[,2], POSTVEC[,c(12:20,24:26)], tol=0.1,method="neuralnet"),T)
...
Z1noLL8 <- try(abc(OBS[1,c(12:20,24:26)], POSTVEC[,5], POSTVEC[,c(12:20,24:26)], tol=0.1,method="loclinear"),T)
...
write(OBS, file=paste("all_pods.obs"),ncol=68,append=T)
